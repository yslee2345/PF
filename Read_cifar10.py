...
class loaddata:
    ...
    def __init__(self,dir,grayscale=True):
        self.dir = dir
    ...
    def _loaddata(self,*filename,one_hot_encoding=True,normalization=True,single_data=False):
        ...
        if single_data:
            ...
            if one_hot_encoding:
                ...
                if normalization:
                    return self.normalization(np.array(x_data)[:,0:array_val].astype(np.float32)), np.array(x_label)
                else:
                    return np.array(x_data)[:, 0:array_val].astype(np.float32), np.array(x_label)
            else:
                x_label = np.array(x_data)[:, array_val]
                if normalization:
                    return self.normalization(np.array(x_data)[:, 0:array_val].astype(np.float32)), x_label
                else:
                    return np.array(x_data)[:, 0:array_val].astype(np.float32), x_label
...
#
# load = loaddata('D:\\Python\\PF\\DATA\\rawdata\\rawdata\\',grayscale=False) #저장경로 입력
